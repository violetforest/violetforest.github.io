...
function init() {
  ...
  stage = new createjs.Stage(canvas, {preserveBuffer:true, antialias:true});
...
function tick(event) {
  ...
  canvas.getContext('2d').fillStyle = "rgba(255, 255, 255, 0.5)";
  ...
  for (var i = 0; i < l; i++) {
    ...
    if (sparkle.y > canvas.height) {
      ...
      sparkle.vX += Math.cos( Math.random() * Math.PI * 2) * 13;
    } else if (sparkle.y < 0) {
      sparkle.vY *= 0.9;
    }
